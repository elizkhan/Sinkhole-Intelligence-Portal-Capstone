# Code adapted from Aaron Geller EarthEngineToGeoTiff repository https://github.com/ageller/EarthEngineToGeoTIFF
# For addtional details go here https://sites.northwestern.edu/researchcomputing/2021/11/19/downloading-satellite-images-made-easy/
import zipfile
import os
import requests
...
def getSentinalS2SRImage(lon, lat, sze, filename, dateMin = '2020-04-01', dateMax = '2020-04-30', vmin = 0, vmax = 3500, j=0):
    '''
    ...
    print(min(nimg,j))
    for i in range(min(nimg,j+1)):
        db = ee.Image(collection.toList(nimg).get(i))
        ...
        if continueFlag:
            ...
            print('Actual filename is {}'.format(filename + '-'+ str(i)+'.tif'))
            with rasterio.open(filename + '-'+ str(i)+'.tif', 'w', **B2_geo) as dest:
